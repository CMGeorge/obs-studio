      ...
      ui             (new Ui::OBSBasic)
{
    ...
    stringstream name;
    name << "Juvenes LIVE (by REEA) " << App()->GetVersionString();
...
void OBSBasic::on_scenes_customContextMenuRequested(const QPoint &pos)
{
    ...
    if (item) {
        ...
        popup.addAction("Open In New Window",this,SLOT(openSceneInNewWindow()));
        popup.addSeparator();
...
{
    QUrl url = QUrl("https://reea.net", QUrl::TolerantMode);
    QDesktopServices::openUrl(url);
}
...
void OBSBasic::OpenProjector(obs_source_t *source, int monitor,bool detached)
{
    ...
    if (!detached){
        ...
        projector = new OBSProjector(nullptr, source);
    }else{
        projector = new OBSProjector(nullptr, source, Qt::WindowFlags());
        projector->setWindowTitle(obs_source_get_name(source));
    }
    const char *name = obs_source_get_name(source);
...
void OBSBasic::openSceneInNewWindow(){
    OBSScene scene = GetCurrentScene();
    if (!scene) return;
    OpenProjector(obs_scene_get_source(scene),0,true);
}
...
void OBSBasic::SystemTrayInit()
{
    trayIcon = new QSystemTrayIcon(QIcon(":/res/images/obs.png"),
                                   this);
    trayIcon->setToolTip("Juvenes Online - Live Studio");
                             ...
                             QSystemTrayIcon::MessageIcon n)
{
    if (QSystemTrayIcon::supportsMessages()) {
        QSystemTrayIcon::MessageIcon icon =
                QSystemTrayIcon::MessageIcon(n);
        trayIcon->showMessage("Juvenes Online Studio", text, icon, 10000);
    }
}
